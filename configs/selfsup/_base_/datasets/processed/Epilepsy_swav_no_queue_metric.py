...
train_pipeline_1 = [  # weak aug
    dict(type='RandomScaling', sigma=0.001, p=0.8),
    dict(type='RandomPermutation', max_segments=5, p=0.8),
    dict(type='RandomJitter', sigma=0.001, p=0.8),
    dict(type='ToTensor'),
]
train_pipeline_2 = [  # strong aug
    dict(type='RandomScaling', sigma=0.001, p=1.0),
    dict(type='RandomPermutation', max_segments=5, p=1.0),
    dict(type='RandomJitter', sigma=0.001, p=1.0),
    dict(type='ToTensor'),
]
...
data = dict(
    ...
    train=dict(
        ...
        num_views=num_crops,
        pipelines=[train_pipeline_1, train_pipeline_2],
        prefetch=prefetch,
    ))
