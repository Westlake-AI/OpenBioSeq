...
class MixFFN(BaseModule):
    ...
    def __init__(self,
        ...
        self.fc1 = _conv_layer(
            in_channels=embed_dims,
            out_channels=feedforward_channels,
            kernel_size=1)
        self.dwconv = _conv_layer(
...
class LKA(BaseModule):
    ...
    def __init__(self, embed_dims, feature_Nd="2d", init_cfg=None):
...
class SpatialAttention(BaseModule):
    ...
    def __init__(self,
        ...
        if feature_Nd == "2d":
            ...
            _conv_layer = nn.Conv1d
        self.proj_1 = _conv_layer(
            in_channels=embed_dims, out_channels=embed_dims, kernel_size=1)
        self.activation = build_activation_layer(act_cfg)
        self.spatial_gating_unit = LKA(embed_dims, feature_Nd=feature_Nd)
        self.proj_2 = _conv_layer(
            in_channels=embed_dims, out_channels=embed_dims, kernel_size=1)
...
class VANBlock(BaseModule):
    ...
    def __init__(self,
        ...
        self.attn = SpatialAttention(
            embed_dims, act_cfg=act_cfg, feature_Nd=feature_Nd)
...
class VANPatchEmbed2d(PatchEmbed):
    ...
    def __init__(self, *args, norm_cfg=dict(type='BN'), **kwargs):
        super(VANPatchEmbed2d, self).__init__(*args, norm_cfg=norm_cfg, **kwargs)
...
class VANPatchEmbed1d(PatchEmbed1d):
    """Image to Patch Embedding of VAN.
    ...
    def __init__(self, *args, norm_cfg=dict(type='BN'), **kwargs):
        super(VANPatchEmbed1d, self).__init__(*args, norm_cfg=norm_cfg, **kwargs)
...
class VAN(BaseBackbone):
    ...
    def __init__(self,
        ...
        if feature_Nd == "2d":
            _build_patch_layer = VANPatchEmbed2d
        elif feature_Nd == "1d":
            _build_patch_layer = VANPatchEmbed1d
        
        cur_block_idx = 0
        for i, depth in enumerate(self.depths):
            patch_embed = _build_patch_layer(
