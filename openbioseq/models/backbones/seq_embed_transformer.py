...
class DNATransformer(BaseBackbone):
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 **kwargs):
        super(DNATransformer, self).__init__(init_cfg)
        ...
        self.pos_embed = nn.Parameter(
            torch.zeros(1, self.seq_len + self.num_extra_tokens, self.embed_dims))
        ...
        if fix_pos_embed:
            pos_emb = build_1d_sincos_position_embedding(
                patches_size=self.seq_len, embed_dims=self.embed_dims,
                cls_token=True)
    ...
    def init_weights(self, pretrained=None):
        super(DNATransformer, self).init_weights(pretrained)
    ...
    def forward(self, x):
        ...
        x = x + resize_pos_embed(
            self.pos_embed,
            src_shape=self.seq_len,
            dst_shape=self.seq_len,
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.out_indices:
                ...
                if self.with_cls_token:
                    patch_token = x[:, 1:].reshape(B, self.seq_len, C)
                    patch_token = patch_token.permute(0, 2, 1)
                    cls_token = x[:, 0]
                else:
                    patch_token = x.reshape(B, self.seq_len, C)
    ...
    def train(self, mode=True):
        super(DNATransformer, self).train(mode)
        self._freeze_stages()
