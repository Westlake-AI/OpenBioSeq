...
class BYOL(BaseModel):
    ...
    *** Requiring Hook: `momentum_update` is adjusted by `CosineScheduleHook`
    ...
    def forward_train(self, data, **kwargs):
        ...
        proj_online_v1 = self.online_net(data_v1)[0]
        proj_online_v2 = self.online_net(data_v2)[0]
        # compute target features
        with torch.no_grad():
            proj_target_v1 = self.target_net(data_v1)[0].detach()
            proj_target_v2 = self.target_net(data_v2)[0].detach()
