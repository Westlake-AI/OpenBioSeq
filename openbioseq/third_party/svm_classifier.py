...
from openbioseq.datasets.utils import to_tensor
    ...
    def evaluate(self,
        ...
        if self.dataset == "onehot" and len(train_targets.shape) == 1:
            train_targets = one_hot(
                to_tensor(train_targets).type(torch.LongTensor), num_classes).numpy()
            test_targets = one_hot(
                to_tensor(test_targets).type(torch.LongTensor), num_classes).numpy()
